...
/**
 * @brief sum of total voltage (bus + shunt)
 * 
 * @param device pointer to device handle
 * @param bus_voltage pointer to store bus voltage in V
 * @param shunt_voltage pointer to store shunt voltage in V
 * @return esp_err_t returns ESP_OK on success
 */
esp_err_t ina226_get_total_voltage(ina226_device_t *device, float *result);
